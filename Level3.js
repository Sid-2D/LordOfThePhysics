...
LoTP.lv3.prototype = {
    ...
    preload: function() {
        ...
        this.addGround();
        this.imp = new Box2D.Common.Math.b2Vec2(0,130);
        this.pt = new Box2D.Common.Math.b2Vec2(1,1);
        ...
        this.back = this.add.sprite(this.world.centerX,this.world.centerY,'back3');
    ...
    addGround : function() {        
        ...
        fixtureDef.restitution = 0.2;             
        fixtureDef.shape = new Box2D.Collision.Shapes.b2PolygonShape();
        fixtureDef.shape.SetAsBox(666/worldScale,0.03);
        fixtureDef.filter.groupIndex=-1;
        
        var bodyDef = new Box2D.Dynamics.b2BodyDef;
        bodyDef.type = Box2D.Dynamics.b2Body.b2_staticBody;
        bodyDef.position.Set(0,471/worldScale);
        
        var body = boxWorld.CreateBody(bodyDef);
        body.CreateFixture(fixtureDef);
        body.SetAngle(0.139626);
        ...
        fixtureDef.restitution = 0.2;             
        fixtureDef.shape = new Box2D.Collision.Shapes.b2PolygonShape();
        fixtureDef.shape.SetAsBox(0.5*180/worldScale,0.03);
        fixtureDef.filter.groupIndex=-1;
        
        bodyDef = new Box2D.Dynamics.b2BodyDef;
        bodyDef.type = Box2D.Dynamics.b2Body.b2_staticBody;
        bodyDef.position.Set(756/worldScale,566/worldScale);
        ...
        fixtureDef.restitution = 0.2;             
        fixtureDef.shape = new Box2D.Collision.Shapes.b2PolygonShape();
        fixtureDef.shape.SetAsBox(0.5*300/worldScale,0.03);
        fixtureDef.filter.groupIndex=-1;
        
        bodyDef = new Box2D.Dynamics.b2BodyDef;
        bodyDef.type = Box2D.Dynamics.b2Body.b2_staticBody;
        bodyDef.position.Set(1130/worldScale,573/worldScale);
    ...
    lUpdate: function(sprite,ptr,dx,dy,snap) {
        if(dx<-100)
            ...
            if(dx<200)
                bodyList[1].GetFixtureList().SetFriction(6);
    },
